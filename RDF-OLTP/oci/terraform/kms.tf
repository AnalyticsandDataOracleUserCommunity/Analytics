...
resource "oci_kms_vault" "techcast_vault" {
  ...
  depends_on = [oci_identity_policy.techcast_basic_policies]
}

# Creates OCI Vault key
resource "oci_kms_key" "techcast_key" {
  compartment_id      = var.compartment_ocid
  display_name        = "${var.vault_key_display_name} - ${random_string.deploy_id.result}"
  management_endpoint = oci_kms_vault.techcast_vault[0].management_endpoint
