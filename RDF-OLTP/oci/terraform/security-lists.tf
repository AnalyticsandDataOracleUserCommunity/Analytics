...
resource "oci_core_security_list" "techcast_security_list" {
  ...
  ingress_security_rules {
    protocol  = local.all_protocols
    source    = lookup(var.network_cidrs, "SECONDARY-SUBNET-REGIONAL-CIDR")
    stateless = true
  }
  ...
  egress_security_rules {
    protocol    = local.all_protocols
    destination = lookup(var.network_cidrs, "SECONDARY-SUBNET-REGIONAL-CIDR")
    stateless   = true
  }
...
resource "oci_core_security_list" "techcast_lb_security_list" {
